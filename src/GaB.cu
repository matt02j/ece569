...
##                      :
## COPYRIGHT            : amrao@email.arizona.edu, harshilpk@email.arizona.edu, priyankag@email.arizona.edu
## ######################################################################################################################## */
#include <stdlib.h> 
#include <string.h> 
#include <math.h> 
...
//#####################################################################################################
//Message from channel is copied from variable node to check node array
...
//#####################################################################################################
//For iterations from 1 and above this kernel is launched to copy message from variable node to check node
...
//##################################################################################################
//This kernel checks if the CtoV copies the same information as VtoC depending on the signe value
...
//#####################################################################################################
//This kernel is launched to decide each check node decision, to decide whether the corresponding variable node is in error or not 
...
//#####################################################################################################
//This kernels sums up the decision made on each variable node depending on all check nodes it is connected to(4 in this case)
...
int main(int argc, char * argv[]) {
  ...
  // Decoder
  // ----------------------------------------------------
  int * CtoV_host, * VtoC_host, * Codeword, * Receivedword_host, * Interleaver_device, * Decide_host, * U, l, kk, * CtoV_device, * VtoC_device, * Receivedword_device, * Decide_device;
  int iter;
  int Synd_host1 = 0;
  int * Synd_host, * Synd_device;
  ...
  cudaMemcpy(Synd_device, Synd_host, M * sizeof(int), cudaMemcpyHostToDevice);
  ...
  for (alpha = alpha_max; alpha >= alpha_min; alpha -= alpha_step) {
    ...
    for (nb = 0, nbtestedframes = 0; nb < NbMonteCarlo; nb++) {
      ...
      for (iter = 0; iter < NbIter; iter++) {
        if (iter == 0) {
          DataPassGBIter0 << < GridDim1, BlockDim1 >>> (VtoC_device, Receivedword_device, Interleaver_device, N, NbBranch);
        } else {
          DataPassGB << < GridDim1, BlockDim1 >>> (VtoC_device, CtoV_device, Receivedword_device, Interleaver_device, N, NbBranch);
        }
        ...
        //Copying Syndrome(Final decision) from GPU to host
        cudaMemcpy(Synd_host, Synd_device, M * sizeof(int), cudaMemcpyDeviceToHost);

        //The error in codeword is detected based on the syndrome value, syndrome=1 indicating error and sydrome=0 indicating no error in codeword. 
        int count1 = 0;
        for (kk = 0; kk < M; kk++) {
          if (Synd_host[kk] == 1)
            count1++;
      ...
      //Copying Decision value from GPu to host for Compute statistics
